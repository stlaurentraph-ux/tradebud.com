...
- **Status**: branch `fix/deploy-smoke-secrets-h23`.
...
- **Status**: merged via PR #309.

### 2026-06-26 (Lane 2 fix — backend tenant scope on DDS package + billing, audit B1–B3)
- **Context**: Production-readiness audit found harvest DDS package endpoints and billing finalize could cross tenant boundaries; admin role bypassed tenant checks.
- **Fixes**:
  - **B1–B2** — `HarvestController` enforces JWT tenant scope on package read/write; admin no longer bypasses tenant isolation.
  - **B3** — `BillingController.finalize` binds checkout to JWT tenant instead of trusting client-supplied tenant id.
- **Tests**: `harvest.controller.spec.ts`, new `billing.controller.spec.ts`, updated `controller-scope.int.spec.ts`.
- **Verify**: backend unit tests 423/423.
- **Status**: merged via PR #308.
...
- **Next** — Optional `EXPO_TOKEN` secret; Phase 4.8 nightly Maestro or Bundle B **1.2**.
...
- **Next** — Slice **3.O.1** blocking Maestro E2E on `main`.
...
- **Next** — Slice **1.O.3** Maestro macOS workflow prep.
