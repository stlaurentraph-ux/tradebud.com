...
| FR-015 | Every Sync now re-downloads all plots/photos despite no cloud changes | Delta cursor not probed; full restore always runs | `field-sync-delta-smoke-guard.mjs` + `field-sync-mode-guard.mjs` + unit tests on evaluate/scope/focus | §11b field-sync-delta |
