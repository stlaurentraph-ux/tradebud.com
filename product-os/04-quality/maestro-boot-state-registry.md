...
| H13 | Golden path preflight: `APK missing assets/maestro/tracebud_offline.db` while assemble verified DB; or stage step `seen[@]: unbound variable` | Stale/wrong `app-debug.apk` at default path; `maestro-ci-stage-android-apk.sh` iterated empty array under `set -u` | `resolve_maestro_apk_path` scans by DB presence; stage script guards empty-array iteration; staged path checked first | Bootstrap resolver + stage script + guard |
