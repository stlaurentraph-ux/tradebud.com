...
| H13 | Golden path preflight: `APK missing assets/maestro/tracebud_offline.db` while assemble verified DB; smoke on same PR passed preflight | Stale/wrong `app-debug.apk` at default path (artifact subdirectory vs expected path); bootstrap trusted first existing file without DB check | `resolve_maestro_apk_path` scans candidates by DB presence; `maestro-ci-stage-android-apk.sh` copies verified APK to `/tmp/tracebud-maestro-ci-app-debug.apk` after download | Bootstrap resolver + workflow stage step + guard |
