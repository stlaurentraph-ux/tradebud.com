...
| H10 | `APK missing assets/maestro/tracebud_offline.db` (smoke on macOS) but DB IS in APK (diagnostic shows it) | `set -o pipefail` + `unzip -l \| grep -q` → `grep -q` exits early on match → `unzip` gets SIGPIPE (exit 141) → pipefail returns rightmost non-zero → false "missing" on macOS (BSD unzip); Ubuntu unzip handles SIGPIPE gracefully so assemble passed | Capture `unzip -l` output in a variable once, then `printf '%s\n' "$list" \| grep -q` — no SIGPIPE | Bootstrap + assemble preflight use captured `APK_LIST` variable |
