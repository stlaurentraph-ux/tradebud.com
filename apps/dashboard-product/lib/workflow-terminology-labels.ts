...
// ─── Campaign Recipient Timeline ─────────────────────────────────────────────

export function getCampaignRecipientTimelineTitle(t?: TranslateFn): string {
  return wf('workflow.campaign_recipient.timeline.title', 'Recipient Timeline', t);
}

export function getCampaignRecipientTimelineDescription(campaignTitle: string | null | undefined, t?: TranslateFn): string {
  const title = campaignTitle?.trim() || 'this campaign';
  return wf(
    'workflow.campaign_recipient.timeline.description',
    `Progress for campaign: ${title}`,
    t,
    { title },
  );
}
...
export function getCampaignRecipientTimelineLoading(t?: TranslateFn): string {
  return wf('workflow.campaign_recipient.timeline.loading', 'Loading recipients…', t);
}

export function getCampaignRecipientTimelineError(t?: TranslateFn): string {
  return wf('workflow.campaign_recipient.timeline.error', 'Failed to load recipient data.', t);
}
...
export function getCampaignRecipientTimelineEmptyRecipients(t?: TranslateFn): string {
  return wf('workflow.campaign_recipient.timeline.empty.recipients', 'No recipients yet.', t);
}

export function getCampaignRecipientTimelineEmptyFiltered(t?: TranslateFn): string {
  return wf('workflow.campaign_recipient.timeline.empty.filtered', 'No recipients match the current filter.', t);
}

export function getCampaignRecipientTimelineEmptyActivity(t?: TranslateFn): string {
  return wf('workflow.campaign_recipient.timeline.empty.activity', 'No activity recorded yet.', t);
}

export function getCampaignRecipientTimelineCopyEmailLabel(t?: TranslateFn): string {
  return wf('workflow.campaign_recipient.timeline.copy_email', 'Copy email address', t);
}

export function getCampaignRecipientTimelineLastActivityLabel(relativeTime: string, t?: TranslateFn): string {
  return wf(
...
export function getCampaignRecipientTimelineLatestDecisionLabel(relativeTime: string, t?: TranslateFn): string {
  return wf(
    'workflow.campaign_recipient.timeline.latest_decision',
    `Latest decision: ${relativeTime}`,
    t,
    { relativeTime },
  );
}
...
export function getCampaignRecipientTimelineFilterLabel(filter: CampaignRecipientFilter, t?: TranslateFn): string {
...
// ─── Campaign Recipient Funnel Summary ───────────────────────────────────────
...
export function getCampaignRecipientFunnelProgressLabel(
  ...
  return wf(
    ...
    values
      ? `${values.fulfilled} of ${values.total} fulfilled (${values.percent}%)`
      : 'Fulfillment progress',
    t,
    values ? { fulfilled: values.fulfilled, total: values.total, percent: values.percent } : undefined,
  );
}

export function getCampaignRecipientProgressStepLabel(
  stepId: CampaignRecipientProgressStepId,
  t?: TranslateFn,
): string {
  ...
  return wf(keyMap[stepId], fallbackMap[stepId], t);
}
...
export function getCampaignRecipientFunnelStatLabel(
  ...
  return wf(keyMap[key], fallbackMap[key], t, values ? { count: values.count } : undefined);
}

// ─── Inbound Campaign Banner ──────────────────────────────────────────────────

export function getInboundCampaignBannerTitle(t?: TranslateFn): string {
  return wf('workflow.inbox.inbound_banner.title', 'You have been invited to a data request', t);
}
...
export function getInboundCampaignBannerMessage(
  ...
  return wf(
    'workflow.inbox.inbound_banner.message',
    values
      ? `${values.fromOrg} has requested "${values.title}" — due ${values.dueDate}.`
      : 'A data request is awaiting your response.',
    t,
    values ? { fromOrg: values.fromOrg, title: values.title, dueDate: values.dueDate } : undefined,
  );
}

export function getInboundCampaignBannerCta(t?: TranslateFn): string {
  return wf('workflow.inbox.inbound_banner.cta', 'Respond now', t);
}

// ─── Outreach Responses Summary ───────────────────────────────────────────────
