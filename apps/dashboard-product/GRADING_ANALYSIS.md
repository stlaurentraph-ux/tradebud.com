...
| **B) Role/Permission Fidelity** | 2 | Canonical Role-Action Matrix (C, lines 58-77) correctly references 9 roles and Section 8 RBAC model; matrix accurately shows permission boundaries (e.g., Field Agent cannot seal shipment). CRITICAL GAP: Matrix uses commercial tier names (Supplier User, Org Admin) instead of legal workflow roles (OPERATOR, DOWNSTREAM_OPERATOR_FIRST, etc.) from Section 5. This conflates commercial tier with legal role—a CRITICAL architectural mismatch per spec Section 5. Missing: role decision engine (Section 6) not addressed; PENDING_MANUAL_CLASSIFICATION workflow not designed. | Hybrid commercial/legal role nomenclature creates implementation ambiguity | Section C (lines 58-77) uses commercial tiers, not legal workflow roles per Section 5 |
| **C) MVP vs V1 Phasing Fidelity** | 3 | Remediation Blueprint explicitly marks routes as MVP vs V1+; Section 51 (MVP Scope Definition) constraints are largely respected: MANUAL_ASSIST-only (✓), no API_DIRECT (✓), no simplified declarations (✓), no downstream/trader workflows (✓), no sponsor governance (✓). Route remediation (Section B, lines 33-55) correctly defers `/requests`, `/sponsor-admin` as Release 2+. Polish spec cleanly separates "Release 2+ extension lane" (informative). | None identified | Route Remediation Plan (lines 33-55), deferred routes clearly marked V1+ |
...
1. **Issue:** Legal Role Model Conflation (Category B)
   - **Problem:** Remediation Blueprint Section C uses commercial tier names (Supplier User, Org Admin, Field Agent) in the role-action matrix instead of canonical legal workflow roles (OPERATOR, DOWNSTREAM_OPERATOR_FIRST, TRADER, etc.) from Section 5.
   - **Impact:** Implementation will create two separate role systems (commercial tier + legal role), violating Section 5 design principle "Legal role is determined per workflow, never as a static organisation label."
   - **Impacted Areas:** All permission checks, shipment sealing logic, DDS submission eligibility, simplified-path gating
   - **Fix Action:** Refactor Section C role-action matrix to use canonical legal workflow roles as primary labels, with commercial tier noted as secondary attribute. Add role decision engine (Section 6) screen/logic to determine legal role per shipment workflow.
   - **Severity:** BLOCKS PHASE 1 BUILD
...
2. **Issue:** request_campaigns Workflow Not Designed (Category D)
   ...
   - **Fix Action:** Design `/requests` page (route does exist in remediation line 54 as deferred, but should be MVP). Include: request creation form, request inbox, request lifecycle timeline, reminder/escalation handling.
   - **Severity:** BLOCKS MVP COMPLIANCE-CRITICAL PATH
...
3. **Design request_campaigns Workflow (/requests Page)**
