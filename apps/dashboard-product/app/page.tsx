...
export default function ConsoleHome() {
  ...
  useEffect(() => {
    const token = window.sessionStorage.getItem('tracebud_exporter_token');
    const storedEmail = window.sessionStorage.getItem('tracebud_exporter_email');
    if (token) {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {loggedInEmail ? (
            <div style={{ fontSize: 12, opacity: 0.9 }}>
              Logged in as <span style={{ fontWeight: 600 }}>{loggedInEmail}</span>
            </div>
          ) : (
          ...
          {loggedInEmail ? (
            <button
              ...
              onClick={async () => {
                ...
                window.sessionStorage.removeItem('tracebud_exporter_token');
                window.sessionStorage.removeItem('tracebud_exporter_email');
                setAccessToken(null);
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    window.sessionStorage.setItem(
                      'tracebud_exporter_token',
                      data.session.access_token,
                    );
                    window.sessionStorage.setItem('tracebud_exporter_email', email.trim());
                    ...
                    {packages.map((p: any) => (
                      <tr key={p.id} style={{ borderBottom: '1px solid #111827' }}>
                        <td style={{ padding: '6px 4px' }}>
                          <Link
                            href={`/packages/${encodeURIComponent(String(p.id))}`}
