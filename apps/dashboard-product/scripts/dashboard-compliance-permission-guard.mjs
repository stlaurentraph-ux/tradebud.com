#!/usr/bin/env node
/**
 * Ensures compliance issues nav permissions and page PermissionGate actions match registry contracts.
 */
...
function extractRegistryContracts(registrySource) {
  const match = registrySource.match(
    /export const DASHBOARD_COMPLIANCE_PAGE_CONTRACTS = \[([\s\S]*?)\] as const;/,
  );
  if (!match) return [];
  const contracts = [];
...
function main() {
  ...
  if (contracts.length === 0) {
    issues.push('DASHBOARD_COMPLIANCE_PAGE_CONTRACTS is empty or unparsable');
  }
  ...
  if (issues.length === 0) {
    console.log(`dashboard-compliance-permission-guard: OK (${contracts.length} contracts)`);
    process.exit(0);
  }

  console.error('dashboard-compliance-permission-guard: FAILED\n');
