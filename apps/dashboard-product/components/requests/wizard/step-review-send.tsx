...
> = {
  ...
  identity_verification: { label: 'Identity Verification', icon: Users },
...
export function StepReviewSend({
  ...
  const typeConfig = requestData.requestType
    ? REQUEST_TYPE_CONFIG[requestData.requestType]
    : null;
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-medium">
                  {format(requestData.dueDate, 'MMMM d, yyyy')}
                </p>
              </div>
            </div>
          )}
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <CardTitle className="text-base">
            Recipients ({recipientsData.selectedRecipients.length})
          </CardTitle>
          <Button
            variant="ghost"
            size="sm"
        ...
        <CardContent>
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            {recipientsByType.organization && (
              <Badge variant="secondary" className="gap-1.5">
                ...
                {recipientsByType.organization.length} Organization
                {recipientsByType.organization.length !== 1 ? 's' : ''}
              </Badge>
            )}
            {recipientsByType.farmer && (
              <Badge variant="secondary" className="gap-1.5">
                <User className="h-3.5 w-3.5" />
                {recipientsByType.farmer.length} Farmer
            ...
            {recipientsByType.plot && (
              <Badge variant="secondary" className="gap-1.5">
                ...
                {recipientsByType.plot.length} Plot
                {recipientsByType.plot.length !== 1 ? 's' : ''}
              </Badge>
            )}
          </div>
          ...
          <Collapsible
            open={recipientsExpanded}
            onOpenChange={setRecipientsExpanded}
          >
            <CollapsibleTrigger asChild>
              ...
              >
                <span>
                  {recipientsExpanded ? 'Hide' : 'Show'} all recipients
                </span>
                {recipientsExpanded ? (
                ...
                {recipientsData.selectedRecipients.map((recipient) => (
                  ...
                  >
                    ...
                    <Badge
                      ...
                      className={cn(
                        'shrink-0 text-xs',
                        recipient.complianceStatus === 'compliant' &&
                          'border-green-200 bg-green-50 text-green-700',
                        recipient.complianceStatus === 'pending' &&
