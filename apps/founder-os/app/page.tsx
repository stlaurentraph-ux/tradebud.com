...
export default function FounderOsHomePage() {
  ...
  const nextActions = actions
    ...
    .map(
      (item) =>
        `${item.action_type} - ${item.prospects?.name ?? getFounderOsCopy('unknown_prospect')}`,
    );
  ...
  return (
    <div className="p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <div>
          <h1 className="text-2xl font-semibold">{getFounderOsCopy('page_title')}</h1>
          <p className="text-sm text-muted-foreground">{getFounderOsCopy('page_subtitle')}</p>
        </div>

        {loading ? (
          <AsyncState mode="loading" title={getFounderOsCopy('loading_title')} />
        ) : error ? (
          <AsyncState mode="error" title={getFounderOsCopy('error_title')} description={error} />
        ) : (
          <>
            <div className="grid gap-4 md:grid-cols-4">
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">{getFounderOsCopy('stat_outreach_today')}</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">{getFounderOsCopy('stat_posts_week')}</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">{getFounderOsCopy('stat_exchanges_week')}</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">{getFounderOsCopy('stat_open_actions')}</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{getFounderOsCopy('auto_plan_title')}</CardTitle>
                <CardDescription>{getFounderOsCopy('auto_plan_description')}</CardDescription>
              </CardHeader>
              <CardContent className="flex flex-wrap gap-2">
                  ...
                  onClick={() => {
                    ...
                    void ensureDailyTarget(today, 3)
                      .then((added) => {
                        ...
                        setActionMessage(
                          added > 0
                            ? getFounderOsCopy('outreach_added', { count: added })
                            : getFounderOsCopy('outreach_not_needed'),
                        );
                      })
                      .catch((err) =>
                        setActionMessage(
                          err instanceof Error ? err.message : getFounderOsCopy('outreach_plan_failed'),
                        ),
                      )
                ...
                >
                  {isPlanningDaily
                    ? getFounderOsCopy('plan_outreach_loading')
                    : getFounderOsCopy('plan_outreach')}
                </Button>
                <Button
                  variant="outline"
                  ...
                  onClick={() => {
                    ...
                    void ensureWeeklyTarget(today, 2)
                      .then((added) => {
                        ...
                        setActionMessage(
                          added > 0
                            ? getFounderOsCopy('posts_added', { count: added })
                            : getFounderOsCopy('posts_not_needed'),
                        );
                      })
                      .catch((err) =>
                        setActionMessage(
                          err instanceof Error ? err.message : getFounderOsCopy('posts_plan_failed'),
                        ),
                      )
                ...
                >
                  {isPlanningWeekly
                    ? getFounderOsCopy('plan_posts_loading')
                    : getFounderOsCopy('plan_posts')}
                </Button>
                <div className="w-full pt-2 text-sm text-muted-foreground">
                  <span className="mr-4">
                    {getFounderOsCopy('outreach_streak', { count: outreachStreak })}
                  </span>
                  <span>{getFounderOsCopy('publishing_streak', { count: postStreak })}</span>
                </div>
                <Button asChild>
                  <Link href="/crm/daily-actions">{getFounderOsCopy('open_outreach_queue')}</Link>
                </Button>
                <Button asChild variant="secondary">
                  <Link href="/content/calendar">{getFounderOsCopy('open_content_board')}</Link>
                </Button>
                <Button asChild variant="outline">
                  <Link href="/strategy/intelligence">GTM Intelligence</Link>
                </Button>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{getFounderOsCopy('next_actions_title')}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {nextActions.length === 0 ? (
                  <p className="text-sm text-muted-foreground">{getFounderOsCopy('next_actions_empty')}</p>
                ) : (
