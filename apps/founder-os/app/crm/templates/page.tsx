  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-6xl space-y-4">
        ...
        {isLoading ? (
          <AsyncState mode="loading" title={getWorkflowAsyncStateCopy('crm.templates', 'loading').title} />
        ) : error ? (
          <AsyncState mode="error" title={getWorkflowAsyncStateCopy('crm.templates', 'error').title} description={error} onRetry={reload} />
        ) : filtered.length === 0 ? (
          <AsyncState mode="empty" title={getWorkflowAsyncStateCopy('crm.templates', 'empty').title} description={getWorkflowAsyncStateCopy('crm.templates', 'empty').description} />
        ) : (
