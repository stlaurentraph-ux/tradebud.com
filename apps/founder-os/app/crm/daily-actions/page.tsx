  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-6xl space-y-4">
        ...
        {isLoading ? (
          <AsyncState mode="loading" title={getWorkflowAsyncStateCopy('crm.daily_actions', 'loading').title} />
        ) : error ? (
          <AsyncState mode="error" title={getWorkflowAsyncStateCopy('crm.daily_actions', 'error').title} description={error} onRetry={reload} />
        ) : actions.length === 0 ? (
          <AsyncState mode="empty" title={getWorkflowAsyncStateCopy('crm.daily_actions', 'empty').title} description={getWorkflowAsyncStateCopy('crm.daily_actions', 'empty').description} />
        ) : (
          actions.map((action) => (
            <Card key={action.id}>
