  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-6xl space-y-4">
        ...
        {isLoading ? (
          <AsyncState mode="loading" title={getWorkflowAsyncStateCopy('content.tasks', 'loading').title} />
        ) : error ? (
          <AsyncState
            mode="error"
            title={getWorkflowAsyncStateCopy('content.tasks', 'error').title}
            description={error}
            onRetry={reload}
          />
        ) : tasks.length === 0 ? (
          <AsyncState
            mode="empty"
            title={getWorkflowAsyncStateCopy('content.tasks', 'empty').title}
            description={getWorkflowAsyncStateCopy('content.tasks', 'empty').description}
          />
        ) : (
          tasks.map((task) => (
            <Card key={task.id}>
