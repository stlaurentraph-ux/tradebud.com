    ...
    <div className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-6xl space-y-4">
        ...
        {isLoading ? (
          <AsyncState
            mode="loading"
            title={getWorkflowAsyncStateCopy('content.calendar', 'loading').title}
          />
        ) : error ? (
          <AsyncState
            mode="error"
            title={getWorkflowAsyncStateCopy('content.calendar', 'error').title}
            description={error}
            onRetry={reload}
          />
        ) : items.length === 0 ? (
          <AsyncState
            mode="empty"
            title={getWorkflowAsyncStateCopy('content.calendar', 'empty').title}
            description={getWorkflowAsyncStateCopy('content.calendar', 'empty').description}
          />
        ) : (
          items.map((item) => (
            <Card key={item.id}>
