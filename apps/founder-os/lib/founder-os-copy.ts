...
export function getFounderOsCopy(
  ...
  tOrValues?: TranslateFn | Record<string, string | number>,
  values?: Record<string, string | number>,
): string {
  const entry = FOUNDER_OS_COPY[key];
  const t = typeof tOrValues === 'function' ? tOrValues : undefined;
  const resolvedValues =
    typeof tOrValues === 'object' && tOrValues !== null ? tOrValues : values;
  return wf(entry.key, entry.fallback, t, resolvedValues);
}
