...
export const NETWORK_ROUTING_FLOWS: readonly NetworkRoutingFlow[] = [
  ...
  },
  {
    ...
    emailResolver: EMAIL_TENANT_RESOLUTION_MODULE,
    preconditions: [
      'crm_contacts.contact_type = farmer for recipient email or phone-only contact',
      'campaign_recipient_invites row with claim token for WhatsApp path',
      'field-app-bootstrap links farmer_profile to auth user',
    ],
    backendModules: [
      'requests/claim-campaign-invites-for-field-farmer.ts',
      'requests/claim-campaign-invite-by-token.ts',
      'requests/campaign-claim-token.ts',
      'requests/campaign-recipient-invite.ts',
    ...
    fieldAppSurfaces: [
      ...
      'apps/offline-product/features/auth/phoneOtpSignIn.ts',
