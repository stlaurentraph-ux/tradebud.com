...
export class BulkPlotImportJobStorageService {
  ...
  async persistJobPayload(params: {
    ...
    if (!uploaded) {
      this.logger.warn(
        `Bulk import job ${params.jobId} exceeded inline threshold but object storage is unavailable; storing inline.`,
      );
      return {
        fileStorageKey: null,
        payloadJsonb: params.payload,
  ...
  async loadJobPayload(params: {
    ...
    fileStorageKey?: string | null;
  }): Promise<BulkPlotImportJobPayload> {
  ...
  private async uploadJson(storagePath: string, body: string): Promise<boolean> {
    ...
    if (error) {
      this.logger.warn(`Bulk import payload upload failed for ${storagePath}: ${error.message}`);
      return false;
    }
    return true;
  }
  ...
  private async downloadJson(storagePath: string): Promise<string | null> {
    ...
    if (error || !data) {
      this.logger.warn(`Bulk import payload download failed for ${storagePath}: ${error?.message ?? 'missing object'}`);
      return null;
    }
    return data.text();
  }
