...
const tenureParseMock = {
  ...
  listForPlot: jest.fn().mockResolvedValue([]),
};
...
describe('PlotsService.runGfwCheck', () => {
  beforeEach(() => {
    gfwMock.runHistoricalDeforestationQuery.mockReset();
    gfwMock.runRaddFallback.mockReset();
  });

  it('persists compliant plot status when no post-cutoff alerts are found', async () => {
    const pool = makePoolMock([
      {
        rows: [{ id: 'plot_1', kind: 'polygon', geojson: '{"type":"Polygon","coordinates":[]}' }],
      },
    ...
    const updateCall = (pool.query as jest.Mock).mock.calls.find((call) =>
  ...
  it('merges GFW red signal with overlap degradation into deforestation_detected', async () => {
    const pool = makePoolMock([
      ...
      { rows: [{ sinaph_overlap: true, indigenous_overlap: true }] },
      { rows: [{ status: 'pending_check', deforestation_screening: null }] },
      { rows: [] },
      { rows: [] },
    ]);
    ...
    expect(result.plotStatus).toBe('deforestation_detected');
  });

  it('keeps under_review when satellite is clear but geo-verified photos are insufficient', async () => {
    const pool = makePoolMock([
      {
        rows: [{ id: 'plot_1', kind: 'polygon', geojson: '{"type":"Polygon","coordinates":[]}' }],
      },
        ...
        rows: [
          {
            total_count: 2,
            geo_tagged_count: 2,
            geo_verified_count: 2,
            timestamp_verified_count: 2,
            clearance_verified_count: 2,
          },
        ],
      },
      { rows: [] },
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ok: true,
        plotStatus: 'under_review',
        reviewClearanceBlocked: true,
        groundTruthPhotos: expect.objectContaining({
          totalCount: 2,
          clearanceVerifiedCount: 2,
