...
describeIfDb('Controller scope integration: farmer ownership enforcement', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query('CREATE SCHEMA IF NOT EXISTS tb_controller_scope_test');
    await pool.query('SET search_path TO tb_controller_scope_test, public');
  ...
  afterAll(async () => {
    await pool.query('DROP SCHEMA IF EXISTS tb_controller_scope_test CASCADE');
    await pool.end();
  });
