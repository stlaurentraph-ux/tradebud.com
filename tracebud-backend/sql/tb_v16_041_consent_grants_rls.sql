-- TB-V16-041: RLS for consent_grants (defense-in-depth; NestJS remains primary gate)
...
USING (
  EXISTS (
    ...
    WHERE fp.id = consent_grants.farmer_id
...
TO authenticated
USING (
  grantee_tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')
);
